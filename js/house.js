...
export function buildHouse(M) {
  ...
  //  Ограждение северного торца — только на ЗАПАДНОЙ половине: у стены дома
  //  проём под марш Л-2, иначе поднявшийся по ступеням упирается в ограждение
  twG.add(box(twW * 0.48, 1.02, 0.07, M.slatDark, [twW * 0.24, hh.terrace + 0.51, -twL + 0.035]));

...
export function buildHouse(M) {
  ...
  // ограждение по южному краю — с разрывом под марш в сад
  tnG.add(box(tnW * 0.32, 1.02, 0.07, M.slatDark, [tnW * 0.16, hh.terrace + 0.51, -0.035]));
  tnG.add(box(tnW * 0.32, 1.02, 0.07, M.slatDark, [tnW * 0.84, hh.terrace + 0.51, -0.035]));
  // ступени с южной террасы на дорожку П-6 (её отвели из-под террасы на v = 1,5…3,05)
  for (let i = 0; i < 5; i++)
    tnG.add(box(2.2, 0.175, 0.33, M.deck,
      [tnW * 0.42, hh.terrace - 0.175 * (i + 0.5), 0.165 + i * 0.33]));
  root.add(tnG);
